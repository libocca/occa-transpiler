...
bool replaceOklByGnuAttribute(std::list<OklAttrMarker>& gnu_markers,
  ...
  // GNU attribute has more diverisity for locations(and it's nigtmare for parser and AST to handle
  // all cases). than standard attribute. After parsing AST GNU will be replaced by CXX NB: there
  // are cases where GNU attribute could not be parsed and embed into AST For example:
...
OklToGnuResult convertOklToGnuAttribute(OklToGnuStageInput input) {
  ...
  auto input_file = input.oklCppSrc;
