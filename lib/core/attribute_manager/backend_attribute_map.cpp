...
bool BackendAttributeMap::handleAttr(const Attr* attr, const Decl* decl, SessionStage& session) {
...
bool BackendAttributeMap::handleAttr(const Attr* attr, const Stmt* stmt, SessionStage& session) {
  std::string name = attr->getNormalizedFullName();
  auto backend = session.getBackend();
...
bool BackendAttributeMap::hasAttrHandler(SessionStage& session, const std::string& name) {
