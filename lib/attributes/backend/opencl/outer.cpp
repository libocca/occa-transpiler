...
__attribute__((constructor)) void registerOpenclOuterAttrBackend() {
    auto ok = registerBackendHandler(TargetBackend::OPENCL, OUTER_ATTR_NAME, handleOuterAttribute);

    if (!ok) {
        SPDLOG_ERROR("[OPENCL] Failed to register {} attribute handler", OUTER_ATTR_NAME);
    }
}
}  // namespace
