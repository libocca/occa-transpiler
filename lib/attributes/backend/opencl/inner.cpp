...
HandleResult handleInnerAttribute(SessionStage& s,
    ...
    auto prefixCode = opencl::buildInnerOuterLoopIdxLine(
...
__attribute__((constructor)) void registerOpenclInnerAttrBackend() {
    auto ok = registerBackendHandler(TargetBackend::OPENCL, INNER_ATTR_NAME, handleInnerAttribute);

    if (!ok) {
        SPDLOG_ERROR("[OPENCL] Failed to register {} attribute handler", INNER_ATTR_NAME);
    }
}
}  // namespace
