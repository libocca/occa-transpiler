...
__attribute__((constructor)) void registerCUDASharedAttrBackend() {
    auto ok = registerBackendHandler(
        TargetBackend::HIP, SHARED_ATTR_NAME, cuda_subset::handleSharedDeclAttribute);
    ok &= registerBackendHandler(
        TargetBackend::HIP, SHARED_ATTR_NAME, cuda_subset::handleSharedTypeAttribute);
    ok &= registerBackendHandler(
        TargetBackend::HIP, SHARED_ATTR_NAME, cuda_subset::handleSharedVarAttribute);
