...
__attribute__((constructor)) void registerOPENMPSharedHandler() {
    auto ok = registerBackendHandler(
        TargetBackend::OPENMP, SHARED_ATTR_NAME, serial_subset::handleSharedDeclAttribute);
    ok &= registerBackendHandler(
        TargetBackend::OPENMP, SHARED_ATTR_NAME, serial_subset::handleSharedTypeAttribute);
    ok &= registerBackendHandler(
        TargetBackend::OPENMP, SHARED_ATTR_NAME, serial_subset::handleSharedVarAttribute);
