...
class AstTraversal : public clang::RecursiveASTVisitor<AstTraversal<SemanticAnalyzer>> {
 public:
  ...
  bool TraverseRecoveryExpr(clang::RecoveryExpr* expr,
                            typename BaseType::DataRecursionQueue* queue = nullptr);

  bool TraverseFunctionDecl(clang::FunctionDecl* funcDecl);
  bool TraverseParmVarDecl(clang::ParmVarDecl* param);
  bool TraverseAttributedStmt(clang::AttributedStmt* attrStmt,
                              typename BaseType::DataRecursionQueue* queue = nullptr);
...
AstTraversal<SemanticAnalyzer>::AstTraversal(SessionStage& stage)
    : _stage(stage), _kernels(), _analyzer() {}
...
template <class SemanticAnalyzer>
bool AstTraversal<SemanticAnalyzer>::TraverseRecoveryExpr(
  clang::RecoveryExpr* expr,
  typename BaseType::DataRecursionQueue* queue) {
...
template <class SemanticAnalyzer>
bool AstTraversal<SemanticAnalyzer>::TraverseAttributedStmt(
  clang::AttributedStmt* attrStmt,
  typename BaseType::DataRecursionQueue* queue) {
