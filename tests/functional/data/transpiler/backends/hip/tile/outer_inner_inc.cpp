...
@kernel void addVectors1(const int entries, const float* a, const float* b, float* ab) {
    for (int i = 0; i < entries; i += 2; @tile(4, @outer, @inner)) {
        ab[i] = add(a[i], b[i]);
    }
}
...
@kernel void addVectors4(const int entries, const float* a, const float* b, float* ab) {
    for (int i = 0; i < entries; i += 1; @tile(4, @outer, @inner, check = true)) {
        ab[i] = add(a[i], b[i]);
    }
}
...
@kernel void addVectors5(const int entries, const float* a, const float* b, float* ab) {
    for (int i = (entries - 12 + 4); i < (entries + 16); i += (entries / 16 + 1); @tile(4, @outer, @inner)) {
        ab[i] = add(a[i], b[i]);
    }
}
...
@kernel void addVectors6(const int entries, const float* a, const float* b, float* ab) {
    for (int i = 0; i < entries; i += 1; @tile(4, @outer(1), @inner(2))) {
        ab[i] = add(a[i], b[i]);
    }
}
...
@kernel void addVectors7(const int entries, const float* a, const float* b, float* ab) {
    for (int i = 0; i < entries; i += 1; @tile(4, @outer(0), @inner(0))) {
        for (int j = 0; j < entries; j += 1; @tile(4, @inner(1), @inner(2))) {
            ab[i] = add(a[i], b[j]);
        }
    }
}
...
@kernel void addVectors8(const int entries, const float* a, const float* b, float* ab) {
    for (int i = (entries - 12 + static_cast<int>(*a)); i < (entries + 16); i += (entries / 16 + 1); @tile(4, @outer(0), @inner(0), check=true)) {
        for (unsigned long long j = (entries - 12 + static_cast<int>(*a)); j < (entries + 16); j += (entries / 16 + 1); @tile(4, @inner(1), @inner(2), check=true)) {
            ab[i] = add(a[i], b[j]);
        }
    }
}
