...
_occa_addVectors0_0(sycl::queue *queue_, sycl::nd_range<3> *range_,
  ...
  queue_->submit([&](sycl::handler &handler_) {
    handler_.parallel_for(*range_, [=](sycl::nd_item<3> item_) {
      {
        int _occa_tiled_i = (entries - 1) - (((4) * 1) * item_.get_group(2));
        {
...
extern "C" [[sycl::reqd_work_group_size(1, 1, 4)]] void
_occa_addVectors1_0(sycl::queue *queue_, sycl::nd_range<3> *range_,
                    const int entries, const float *a, const float *b,
                    float *ab) {
  queue_->submit([&](sycl::handler &handler_) {
    handler_.parallel_for(*range_, [=](sycl::nd_item<3> item_) {
      {
        int _occa_tiled_i = (entries - 1) - (((4) * 2) * item_.get_group(2));
        {
...
extern "C" [[sycl::reqd_work_group_size(1, 1, 4)]] void
_occa_addVectors2_0(sycl::queue *queue_, sycl::nd_range<3> *range_,
                    const int entries, const float *a, const float *b,
                    float *ab) {
  queue_->submit([&](sycl::handler &handler_) {
    handler_.parallel_for(*range_, [=](sycl::nd_item<3> item_) {
      {
        int _occa_tiled_i = (entries - 1) - ((4) * item_.get_group(2));
        {
...
extern "C" [[sycl::reqd_work_group_size(1, 1, 4)]] void
_occa_addVectors3_0(sycl::queue *queue_, sycl::nd_range<3> *range_,
                    const int entries, const float *a, const float *b,
                    float *ab) {
  queue_->submit([&](sycl::handler &handler_) {
    handler_.parallel_for(*range_, [=](sycl::nd_item<3> item_) {
...
extern "C" [[sycl::reqd_work_group_size(1, 1, 4)]] void
_occa_addVectors4_0(sycl::queue *queue_, sycl::nd_range<3> *range_,
                    const int entries, const float *a, const float *b,
                    float *ab) {
  queue_->submit([&](sycl::handler &handler_) {
    handler_.parallel_for(*range_, [=](sycl::nd_item<3> item_) {
...
extern "C" [[sycl::reqd_work_group_size(1, 1, 4)]] void
_occa_addVectors5_0(sycl::queue *queue_, sycl::nd_range<3> *range_,
                    const int entries, const float *a, const float *b,
                    float *ab) {
  queue_->submit([&](sycl::handler &handler_) {
    handler_.parallel_for(*range_, [=](sycl::nd_item<3> item_) {
...
_occa_addVectors6_0(sycl::queue *queue_, sycl::nd_range<3> *range_,
  ...
  queue_->submit([&](sycl::handler &handler_) {
    handler_.parallel_for(*range_, [=](sycl::nd_item<3> item_) {
      {
        int _occa_tiled_i = (entries - 1) - (((4) * 1) * item_.get_group(1));
        {
    ...
    handler_.parallel_for(*range_, [=](sycl::nd_item<3> item_) {
      {
        int _occa_tiled_i = (entries - 1) - (((4) * 1) * item_.get_group(2));
        {
          ...
          if (i >= 0) {
            {
              int _occa_tiled_j =
                  (entries - 1) - (((4) * 1) * item.get_local_id(1));
              {
